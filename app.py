...
# Initialize Engine (Cached to prevent reload crashes)
@st.cache_resource
def get_engine():
    return SalesStrategixEngine()
...
<div class="main-header">
    ...
    <p>Smart Playbook Companion for Sales & Business Analytics</p>
</div>
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        ...
        if "sources" in message:
            ...
            st.markdown(f'<div class="source-container">{sources_html}</div>', unsafe_allow_html=True)
...
<div class="footer-text">
    Powered by SalesStrategix AI • FAISS Vector Engine • Groq LLaMA3
</div>
""", unsafe_allow_html=True)
